...
def run_melconv(midi_file_in, mcsv_out, melconv_loc="melconv"):
    midi_file_name = os.path.split(midi_file_in)[list_tail].split('.')[
        list_head]
    mcsv_output_path = os.path.split(mcsv_out)[list_head]
    mcsv_output_file_name = os.path.split(mcsv_out)[list_tail].split('.')[
        list_head]
    ...
    if mcsv_out:
        ...
        if len(os.path.split(mcsv_out)[list_tail].split(
                '.')) is 1 or os.path.split(mcsv_out)[list_tail].split(
                    '.')[1] is not 'csv':
            print 'Output not type csv... correcting to ' + mcsv_output_file_name + '.csv'
    ...
    if os.getcwd() is '/Users/cg306/repos/virtuobass/virtuobass/parser/':
        command = "./" + melconv_loc + " -f -s csv -i " + midi_file_in  #+ " -o " + mcsv_out
    else:
        command = "./parser/" + melconv_loc + " -f csv -s -i " + midi_file_in  #+ " -o " + mcsv_out
