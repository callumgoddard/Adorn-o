...
from parser.API.calculate_functions import (
    ...
    calculate_grace_note_possitions, calculate_midi_file_for_measure_note_list,
    calculate_rhy_file_for_measure, calculate_playing_complexity,
    calculate_heuristic)
...
sorted = namedtuple("Sorted",
                    ['adorned', 'complexity_weight', 'difficulty_weight'])

VirtuosityThreshold = namedtuple("VirtuosityThreshold",
                                 ["complexity", "difficulty", 'pieces'])
candidate = namedtuple("Candidates", ['complexity', 'difficulty', 'id'])
retieved = namedtuple('Retrieved', ['id', 'measure'])
modified_notes = namedtuple("ModifiedNotes",
                            ['measure', 'total_modified', 'proportion'])
...
class Database():
    ...
    def __init__(self, save_folder='./', new_database=False, weight_set='GMS'):
        ...
        self.header = [
            'file.id', "file.location", "artist", "song.title", "track",
            "measure.number", "complexity", "perceived.difficulty",
            "unadorned.complexity", "unadorned.perceived.difficulty",
            "single.note", "single.note.pitch", "single.note.fret.number",
            "single.note.string.number", "single.note.duration",
            "mean.entropy", "mean.productivity", "mean.Simpsons.D",
            "mean.Yules.K", "mean.Sichels.S", "mean.Honores.H", "p.range",
            "p.entropy", "p.std", "i.abs.range", "i.abs.mean", "i.abs.std",
            "i.mode", "i.entropy", "d.range", "d.median", "d.mode",
            "d.entropy", "d.eq.trans", "d.half.trans", "d.dotted.trans", "len",
            "glob.duration", "note.dens", "tonalness", "tonal.clarity",
            "tonal.spike", "int.cont.grad.mean", "int.cont.grad.std",
            "int.cont.dir.change", "step.cont.glob.var", "step.cont.glob.dir",
            "step.cont.loc.var", "mode", "h.contour", "int.cont.glob.dir",
            "int.contour.class", "PRS.mean_syncopation_per_bar",
        ...
        if os.path.isfile(self.save_folder + "/sorted.txt"):
            ...
            if sorted_info == []:
                ...
                self.sorted = sorted(False, float(sorted_info[1]),
                                     float(sorted_info[2]))
            elif sorted_info[0] == 'True':
                self.sorted = sorted(True, float(sorted_info[1]),
                                     float(sorted_info[2]))
        else:
            self.sorted = sorted(None, None, None)
    ...
    def make_new_database(self):
        ...
        with open(
                self.save_folder + "/sorted.txt", mode='w') as sorted_savefile:
    ...
    def add_entries_from_list_of_gp5_files(
        ...
        assert isinstance(list_of_gp5_files,
                          list), "list_of_gp5_files is not a list"
    ...
    def add_entries_from_gp5_file(self,
                                  gp5_file,
                                  convert_to_json=True,
        ...
        if move_tabs:
            api_song_data = self.load_gp_file(self.save_folder + "/" +
                                              gp5_file)
        else:
        ...
        for part in gp5_file.split('/')[-1].split('.')[:-1]:
            ...
            if gp5_file.split('/')[-1].split('.')[:-1].index(part) != len(
                    gp5_file.split('/')[-1].split('.')[:-1]) - 1:
                song_file_name_no_path_no_ext += '.'
        ...
        if convert_to_json:
                    ...
                    for measure in track.measures:
                        measures_with_processed_tied_notes.append(
                            Measure(
                                ...
                                notes=notes_in_measure[measure.meta_data.number
                                                       - 1]))
            ...
            if os.path.isfile(self.save_folder + '/json/' +
                              song_file_name_no_path_no_ext + '.json'):

                json_file = str("json/" + song_file_name_no_path_no_ext + " " +
                                str(uuid.uuid4())[:8] + ".json")
            ...
            # Check the JSON file:
            json_api_song_data = self.load_file(self.save_folder + '/' +
                                                json_file)
    ...
    def add_entries_from_list_of_json_files(
            ...
            save_feature_files=False,
            artist_and_title_from_file_name=True):
        assert isinstance(list_of_json_files,
                          list), "list_of_json_files is not a list"
    ...
    def add_entries_from_json_file(self,
                                   json_file,
                                   move_tabs=True,
                                   remove_duplicates=True,
                                   save_feature_files=False,
        ...
        if move_tabs:
            # move the tab
            if not os.path.isfile(self.json_location + "/" +
                                  json_file.split('/')[-1]):
        ...
        if move_tabs:
            api_song_data = self.load_json_file(self.save_folder + "/" +
                                                json_file)
        else:
    ...
    def add_entries(self,
        ...
        for track in range(0, len(api_song_data)):
            ...
            for measure in api_song_data[track].measures:
                ...
                measure_id = self.measure_id(track_id,
                ...
                if (measure_complexities.adorned == {}
                        or measure_complexities.adorned is None):
                    print("No complexity could be calculated for %s" %
                          measure_id)
                    continue
                ...
                song_entry_info.append([
                    measure_id, database_file_location, artist, song_title,
                    track, measure.meta_data.number,
                    measure_complexities.adorned[measure.meta_data.number].BGM,
                    measure_complexities.adorned[measure.meta_data.number].EVC,
                    measure_complexities.unadorned[measure.meta_data.number]
                    .BGM, measure_complexities.unadorned[
                        measure.meta_data.number].EVC, single_note, note_pitch,
                    note_fret_number, note_string_number, note_duration
                ])
                ...
                midi_file = calculate_midi_file_for_measure_note_list(
                    notes_in_measure[measure_number],
                    measure,
                    midi_file_name=measure_id)
                mcsv_file = run_melconv(midi_file,
                                        midi_file.split('.')[0] + ".csv")
            ...
            if feature_df is not False:
                ...
                db_entry = robjects.r['merge'](
                    feature_analysis.read_in_feature_dataframe(
                        song_entry_file_location), feature_df)

                # write out the data_base:
                robjects.r['write.table'](
                    x=db_entry,
                    file=song_entry_file_location,
                    sep=",",
                    row_names=False)
            else:
            ...
            with song_entry_file:
                ...
                for row in song_entry_info:
                    if row == [
                            'file.id', "file.location", "artist", "song.title",
                            "track", "measure.number", "complexity",
                            "perceived.difficulty", "unadorned.complexity",
                            "unadorned.perceived.difficulty", "single.note",
                            "single.note.pitch", "single.note.fret.number",
                            "single.note.string.number", "single.note.duration"
                    ]:
                        continue
                    elif row[10] is True:
                        ...
                        for fantastic_feature in [
                                "mean.entropy", "mean.productivity",
                                "mean.Simpsons.D", "mean.Yules.K",
                                "mean.Sichels.S", "mean.Honores.H", "p.range",
                                "p.entropy", "p.std", "i.abs.range",
                                "i.abs.mean", "i.abs.std", "i.mode",
                                "i.entropy", "d.range", "d.median", "d.mode",
                                "d.entropy", "d.eq.trans", "d.half.trans",
                                "d.dotted.trans", "len", "glob.duration",
                                "note.dens", "tonalness", "tonal.clarity",
                                "tonal.spike", "int.cont.grad.mean",
                                "int.cont.grad.std", "int.cont.dir.change",
                                "step.cont.glob.var", "step.cont.glob.dir",
                                "step.cont.loc.var", "mode", "h.contour",
                                "int.cont.glob.dir", "int.contour.class"
                        ]:
                        ...
                        if row[0] + ".rhy" in rhy_files:
                            ...
                            synpy_data = feature_analysis.synpy_interface.compute_features(
                                row[0] + ".rhy")[1:]
                            print(song_entry_info[0])
                            print(row + dummy_fantastic_data + synpy_data)
                            assert len(row + dummy_fantastic_data +
                                       synpy_data) == len(self.header)
                            writer.writerow(row + dummy_fantastic_data +
                                            synpy_data)
                    else:
                        ...
                        if row[0] in two_note_measures:
                            ...
                            for fantastic_feature in [
                                    "mean.entropy", "mean.productivity",
                                    "mean.Simpsons.D", "mean.Yules.K",
                                    "mean.Sichels.S", "mean.Honores.H",
                                    "p.range", "p.entropy", "p.std",
                                    "i.abs.range", "i.abs.mean", "i.abs.std",
                                    "i.mode", "i.entropy", "d.range",
                                    "d.median", "d.mode", "d.entropy",
                                    "d.eq.trans", "d.half.trans",
                                    "d.dotted.trans", "len", "glob.duration",
                                    "note.dens", "tonalness", "tonal.clarity",
                                    "tonal.spike", "int.cont.grad.mean",
                                    "int.cont.grad.std", "int.cont.dir.change",
            ...
            if save_feature_files is False:
                ...
                if not os.path.isdir(self.save_folder + "/midi_files/" +
                                     track_id):
                    os.makedirs(self.save_folder + "/midi_files/" + track_id)

                if not os.path.isdir(self.save_folder + "/rhy_files/" +
                                     track_id):
                    os.makedirs(self.save_folder + "/rhy_files/" + track_id)

                if not os.path.isdir(self.save_folder + "/csv_files/" +
                                     track_id):
                ...
                for fname in midi_files:
                    if os.path.isfile(fname):
                        shutil.move(
                            fname,
                            self.save_folder + "/midi_files/" + track_id)

                for rhy_file in rhy_files:
                    if os.path.isfile(rhy_file):
                        shutil.move(
                            rhy_file,
                            self.save_folder + "/rhy_files/" + track_id)
    ...
    def valid_measure(self, measure, notes_in_measure):
        ...
        if measure.meta_data.monophonic is False:
            print("Measure %s is not monophonic, skipping..." %
                  measure.meta_data.number)
            return False

        # - contain notes
        if notes_in_measure == []:
            print("Measure %s is has no notes, skipping..." %
                  measure.meta_data.number)
            return False
    ...
    def add_entry(self, song_entry_file_location, remove_duplicates=True):
        ...
        assert isinstance(
            ...
            str), ("%s is not a string" % song_entry_file_location)
        assert song_entry_file_location.split('.')[-1] == "csv", (
            "%s is does not have csv file extension" %
            song_entry_file_location)
        ...
        with open(str(song_entry_file_location)) as song_entry_file:
            ...
            for entry in song_entry:
                ...
                if line_count == 0:
                    ...
                    if os.path.isfile(self.save_folder + '/' +
                                      entry[1]) or os.path.isfile(entry[1]):
    ...
    def retrieve_data_form_multiple_entries_from_same_files(
        ...
        for entry_id in entry_ids:
            ...
            if entry:
                ...
                measure = int(
                    entry[self.header.index('measure.number') - 1]) - 1
                """
    ...
    def retrieve_data_from_multiple_entries(self, entry_ids, return_ids=False):
        ...
        '''
        files_to_load = self.consolidate_multiple_entries_from_same_files(
            entry_ids)
        ...
        for file_location in files_to_load:
            return_measures += (
                self.retrieve_data_form_multiple_entries_from_same_files(
                    file_location,
                    files_to_load[file_location],
    ...
    def retrieve_entry_data(self, entry_id):
        ...
        try:
            ...
            try:
                ...
                if entry:
                    file_location = entry[self.header.index('file.location') -
                                          1]
                    track = int(entry[self.header.index('track') - 1])
                    measure = int(
                        entry[self.header.index('measure.number') - 1]) - 1
    ...
    def sort(self,
        ...
        if (self.sorted.adorned == adorned
        ...
        if save:
            ...
            with open(
                    self.save_folder + "/" + "sorted.txt",
                    mode="w") as sorted_savefile:
    ...
    def subset(self,
        ...
        if lower_percentile > 0 or upper_percentile < 100:
            ...
            elif (self.sorted.complexity_weight != complexity_weight
        ...
        artists = [
            artist for artist in artists if artist not in exclude_artists
        ]
        files = [file for file in files if file not in exclude_files]

        return subset_database(database_features, lower_percentile,
                               upper_percentile,
    ...
    def find_most_similar(self,
        ...
        similarity_threshold = 100 - subset_parameters.get(
        ...
        # Load r related things:
        feature_analysis.fantastic_interface.load()
        if self.get_similarity_features_for_measure(measure,
                                                    notes_in_measure) is None:
            return []
    ...
    def sort_candidate_set(self,
        ...
        if adorned:
            ...
            difficulty_index = self.header.index(
                'unadorned.perceived.difficulty') - 1
    ...
    def get_similarity_features_for_measure(self, measure,
                                            notes_in_measure=[]):
        '''
        ...
        if not self.valid_measure(
                measure=measure, notes_in_measure=notes_in_measure):
            return None
        ...
        if len(notes_in_measure) == 1:
            ...
            fantastic_features = single_note_features(
                measure_id, notes_in_measure[0].note.pitch,
            ...
            feature_df = robjects.r['merge'](fantastic_features,
                                             synpy_features)

        if len(notes_in_measure) == 2:
            print("Two note measure")
            feature_df = feature_analysis.combine_synpy_and_fantastic_features_for_two_notes(
    ...
    def general_virtuosity_threshold(self, virtuosity_percentile,
                                     complexity_weight, difficulty_weight):
        """
        ...
        adorned_threshold = sorted_adorned_db[adorned_percentile_index][
            self.header.index("complexity"):self.header.index(
    ...
    def virtuosity_threshold(self,
        ...
        # sort out the subset parameters:
        percentile_range = other_similarity_parameters.get(
            'percentile_range', [0, 100])
        #adorned = subset_parameters.get('adorned', True)
        artists = other_similarity_parameters.get('artists', 'all')
        files = other_similarity_parameters.get('files', 'all')
        exclude_artists = other_similarity_parameters.get(
            'exclude_artists', ['none'])
        exclude_files = other_similarity_parameters.get(
            'exclude_files', ['none'])
        ...
        print("Similarity Matches:", len(consolidated), "Percentile Index:",
              percentile_index)
        print("Virtuosity Threshold:",
        ...
        return VirtuosityThreshold(
            ...
            float(consolidated[percentile_index].difficulty), consolidated)
    ...
    def produce_report(self, input_file, output_file, **parameters):
        ...
        report_name = self.save_folder + '/output/reports/' + output_file.split(
            '/')[-1].split('.gp5')[0] + '_report' + '.csv'
        ...
        report_data.append(
            ['complexity_weight:',
             parameters.get('complexity_weight', '')])
        ...
        similarity_threshold_limit = parameters.get(
            'similarity_threshold_limit', '')
        ...
        for input_track, output_track in zip(input_song, output_song):
            ...
            report_data.append([
                'full_unadorned', input_song_complexity.unadorned.BGM,
                input_song_complexity.unadorned.EVC
            ])
            report_data.append([
                'full_input', input_song_complexity.adorned.BGM,
                input_song_complexity.adorned.EVC
            ])
            report_data.append([
                'full_output', output_song_complexity.BGM,
                output_song_complexity.EVC
            ])
            report_data.append([
                'track', 'measure_number', 'unadorned_complexity',
                'input_complexity', 'output_complexity',
                'unadorned_difficulty', 'input_difficulty',
                'output_difficulty', 'virtuosity_threshold_complexity',
                'virtuosity_threshold_difficulty',
                'passed_virtuosity_threshold', 'total_notes',
                'number_modified', 'modified_proportion'
            ])
            ...
            for input_song, output_song in zip(input_track, output_track):
                ...
                for input_measure, output_measure in zip(
                    ...
                    for input_note, output_note in zip(
                        ...
                        assert (input_note.note.note_number ==
                                output_note.note.note_number)
                        assert (
                            input_note.note.pitch == output_note.note.pitch)
                        assert (input_note.note.start_time ==
                                output_note.note.start_time)
                    ...
                    mod_notes = modified_notes(
                        input_measure.meta_data.number, modified_note_count,
                        modified_note_count / len(
                            input_notes_in_measure[measure_index]))
                    ...
                    if calculate_heuristic(
                            output_song_bar_complexities.adorned[measure_index
                                                                 + 1].BGM,
                    ...
                    report_data.append([
                        track_count, measure_index + 1,
                        input_song_bar_complexities.unadorned[measure_index +
                                                              1].BGM,
                        input_song_bar_complexities.unadorned[measure_index +
                                                              1].EVC,
                        input_song_bar_complexities.adorned[measure_index +
                                                            1].BGM,
                        input_song_bar_complexities.adorned[measure_index +
                                                            1].EVC,
                        output_song_bar_complexities.adorned[measure_index +
                        ...
                        mod_notes.total_modified, mod_notes.proportion
                    ])
            ...
            report_data.append([
                '', '', '', '', '', '', '', '', '', 'totals:',
                total_meaures_passed_virtuosity_threshold, total_notes,
                total_modified_notes, total_modified_notes / total_notes
            ])
            track_count += 1
