...
n.grams.across.melodies <- function(melody.filenames,n.lim=n.limits,phr.length.lim=phr.length.limits,write.out=FALSE) {
	...
	comb.ngr.tab.collapsed <- tapply(ngr.tab$count,ngr.tab$ngram,sum)
	comb.ngr.tab.collapsed <- data.frame(ngram=names(comb.ngr.tab.collapsed),count=comb.ngr.tab.collapsed)
	rownames(comb.ngr.tab.collapsed) <- NULL
	switch(write.out, collapsed=write.table(comb.ngr.tab.collapsed,file="ngram_counts_several_melodies_collapsed.csv",sep=";",row.names=FALSE), full=write.table(ngr.tab,file="ngram_counts_several_melodies.csv",sep=";",row.names=FALSE), both=write.both(comb.ngr.tab.collapsed,ngr.tab))
	comb.ngr.tab.collapsed
	}

write.both <- function(comb.ngr.tab.collapsed,ngr.tab) {
	write.table(comb.ngr.tab.collapsed,file="ngram_counts_several_melodies_collapsed.csv",sep=";",row.names=FALSE)
	write.table(ngr.tab,file="ngram_counts_several_melodies.csv",sep=";",row.names=FALSE)}
