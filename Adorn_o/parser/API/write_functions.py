                        ...
                        for beat in voice.beats:
                            for note in beat.notes:
                                ...
                                if (
                                    ...
                                    or note.type == guitarpro.NoteType.dead
                                ):
                                    # update the note information:
                                    for api_note in apisong.measures[
                                        measure_count
                                    ].notes:
                                        if isinstance(api_note, datatypes.AdornedNote):
                                            if api_note.note.note_number == note_count:
                                                ...
                                                if previous_beat is not None:
                                                    if (
                                                        api_note.adornment.fretting.technique
                                                        == "hammer-on"
                                                        or api_note.adornment.fretting.technique
                                                        == "pull-off"
                                                    ):

                                                        for (
                                                            prev_note
                                                        ) in previous_beat.notes:
                                                            if (
                                                                prev_note.string
                                                                == note.string
                                                            ):
                                                                prev_note.effect.hammer = (
                                                                    True
                                                                )
                                                                break
                                                    elif (
...
def adornment_to_gp5_note(adorned_note, beat, note):
    assert isinstance(
        adorned_note, datatypes.AdornedNote
    ), "Must have adorned note as input"
    adornment = adorned_note.adornment

    assert isinstance(
        adornment, datatypes.Adornment
    ), "adorned_note adornment is not an Adornment"
...
def plucking_modification_to_gp5_note(adorned_note, beat, note):
    ...
    if isinstance(
        adornment.plucking.modification.artificial_harmonic,
...
def modulation_to_gp5_note(adornment, beat, note):
    ...
    if adornment.fretting.modulation.bend is not None:
        ...
        for point in adornment.fretting.modulation.bend.points:
            bend_points.append(
                guitarpro.models.BendPoint(point.position, point.value, point.vibrato)
            )
