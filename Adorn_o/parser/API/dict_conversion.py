...
def Song_to_dict(api_song):
    ...
    return {
        "song": OrderedDict(
            [
                ("meta_data", SongMetaData_to_dict(api_song.meta_data)),
...
def dict_to_SongMetaData(meta_data_dict):
    ...
    return datatypes.SongMetaData(
        ...
        meta_data_dict["bpm"],
        str(meta_data_dict["key"]),
        meta_data_dict["number_of_measures"],
    )
...
def Measure_to_dict(measure):
    ...
    return OrderedDict(
        [
            ("meta_data", MeasureMetaData_to_dict(measure.meta_data)),
            ("start_time", fraction_to_dict(measure.start_time)),
            ("notes", notes),
        ]
    )
...
def dict_to_Measure(measure_dict):
    ...
    return datatypes.Measure(
        ...
        dict_to_fraction(measure_dict["start_time"]),
        notes,
    )
...
def MeasureMetaData_to_dict(meta_data):
    ...
    return OrderedDict(
        [
            ...
            ("key_signature", meta_data.key_signature),
            ("time_signature", meta_data.time_signature),
            ("tempo", meta_data.tempo),
            ("triplet_feel", meta_data.triplet_feel),
            ("monophonic", meta_data.monophonic),
            ("only_tied_notes", meta_data.only_tied_notes),
            ("only_rests", meta_data.only_rests),
        ]
    )
...
def dict_to_MeasureMetaData(meta_data_dict):
    ...
    return datatypes.MeasureMetaData(
        meta_data_dict["title"],
        meta_data_dict["number"],
        str(meta_data_dict["key_signature"]),
        str(meta_data_dict["time_signature"]),
        meta_data_dict["tempo"],
        meta_data_dict["triplet_feel"],
        meta_data_dict["monophonic"],
        meta_data_dict["only_tied_notes"],
        meta_data_dict["only_rests"],
    )
...
def NotatedDuration_to_dict(notated_duration):
    ...
    return OrderedDict(
        [
            ("value", fraction_to_dict(notated_duration.value)),
...
def Tuplet_to_dict(tuplet):
    ...
    return OrderedDict(
        [("number_of_notes", tuplet.number_of_notes), ("equal_to", tuplet.equal_to)]
    )


def dict_to_Tuplet(tuplet_dict):
    return datatypes.Tuplet(tuplet_dict["number_of_notes"], tuplet_dict["equal_to"])
...
def Rest_to_dict(rest):
    ...
    return {
        "rest": OrderedDict(
            [
                ("note_number", rest.note_number),
                ("start_time", fraction_to_dict(rest.start_time)),
                ("duration", fraction_to_dict(rest.duration)),
                ("notated_duration", NotatedDuration_to_dict(rest.notated_duration)),
            ]
        )
    }
...
def AdornedNote_to_dict(note):
    ...
    return {
        "adorned_note": OrderedDict(
            [
                ("note", Note_to_dict(note.note)),
...
def Note_to_dict(note):
    ...
    return OrderedDict(
        [
            ...
            ("pitch", note.pitch),
            ("fret_number", note.fret_number),
            ("string_number", note.string_number),
            ("string_tuning", note.string_tuning),
            ("start_time", fraction_to_dict(note.start_time)),
            ("duration", fraction_to_dict(note.duration)),
            ("notated_duration", NotatedDuration_to_dict(note.notated_duration)),
            ("dynamic", Dynamic_to_dict(note.dynamic)),
        ]
    )
...
def Dynamic_to_dict(dynamic):
    ...
    return OrderedDict([("value", dynamic.value), ("cres_dim", dynamic.cres_dim)])
...
def Adorment_to_dict(adornment):
    ...
    return OrderedDict(
        [
            ("plucking", PluckingAdornment_to_dict(adornment.plucking)),
            ("fretting", FrettingAdornment_to_dict(adornment.fretting)),
            ("grace_note", GraceNote_to_dict(adornment.grace_note)),
            ("ghost_note", adornment.ghost_note),
        ]
    )
...
def PluckingAdornment_to_dict(plucking):
    ...
    return OrderedDict(
        [
            ("technique", plucking.technique),
            ("modification", PluckingModification_to_dict(plucking.modification)),
...
def PluckingModification_to_dict(plucking_mod):
    ...
    return OrderedDict(
        [
            ("palm_mute", plucking_mod.palm_mute),
            (
                "artificial_harmonic",
                ArtificialHarmonic_to_dict(plucking_mod.artificial_harmonic),
            ),
        ]
    )
...
def FrettingAdornment_to_dict(fretting):
    ...
    return OrderedDict(
        [
            ("technique", fretting.technique),
            ("modification", FrettingModification_to_dict(fretting.modification)),
            ("accent", fretting.accent),
            ("modulation", Modulation_to_dict(fretting.modulation)),
        ]
    )
...
def dict_to_FrettingAdornment(fretting_dict):
    ...
    return datatypes.FrettingAdornment(
        fretting_technique,
        dict_to_FrettingModification(fretting_dict["modification"]),
        fretting_dict["accent"],
...
def FrettingModification_to_dict(fretting_modification):
    ...
    return OrderedDict(
        [
            ("type", fretting_modification.type),
...
def dict_to_FrettingModification(fretting_modification_dict):
    ...
    return datatypes.FrettingModification(
        fretting_modification_dict["type"], fretting_modification_dict["let_ring"]
    )
...
def Modulation_to_dict(modulation):
    ...
    return OrderedDict(
        [
            ("bend", Bend_to_dict(modulation.bend)),
...
def dict_to_Modulation(modulation_dict):
    ...
    return datatypes.Modulation(
        dict_to_Bend(modulation_dict["bend"]),
        modulation_dict["vibrato"],
...
def Bend_to_dict(bend):
    ...
    return OrderedDict(
        [("type", bend_type), ("value", bend_value), ("points", bend_points)]
    )
...
def dict_to_Bend(bend_dict):
    if bend_dict["type"] is None:
        ...
        return datatypes.Bend(
            type=bend_dict["type"], value=bend_dict["value"], points=bend_points
        )
...
def BendPoint_to_dict(bend_point):
    ...
    return OrderedDict(
        [
            ("position", bend_point.position),
            ("value", bend_point.value),
            ("vibrato", bend_point.vibrato),
        ]
    )


def dict_to_BendPoint(bend_point_dict):
    return datatypes.BendPoint(
        bend_point_dict["position"],
...
def Trill_to_dict(trill):
    ...
    return OrderedDict([("fret", fret), ("notated_duration", notated_duration)])
...
def dict_to_Trill(trill_dict):
    ...
    if check_none_value(trill_dict):
        ...
        return datatypes.Trill(
            trill_dict["fret"], dict_to_NotatedDuration(trill_dict["notated_duration"])
        )
...
def dict_to_GraceNote(grace_note_dict):
    ...
    if check_none_value(grace_note_dict):
        ...
        return datatypes.GraceNote(
            grace_note_dict["fret"],
