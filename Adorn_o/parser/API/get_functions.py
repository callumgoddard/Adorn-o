...
from .calculate_functions import (
    ...
    calculate_triplet_feel_note_durations,
...
def get_API_from_JSON_file(json_file):
    """Reads in a JSON file and returns the API data"""
    with open(json_file) as read_file:
        json_data = json.load(read_file)
    return get_from_JSON(json_data)
...
def get_from_JSON(json_dict):
    ...
    if "tracks" in list(json_dict.keys()):
        ...
        "notes" in list(json_dict.keys())
        and "start_time" in list(json_dict.keys())
...
def get_rest_data(note_number, beat):
    ...
    return datatypes.Rest(
        note_number, get_start_time(beat), None, get_beat_duration(beat)
    )
...
def get_start_time(beat_or_measure):
    ...
    if type(beat_or_measure) == guitarpro.models.Measure:
        return Fraction(beat_or_measure.header.start, 960) / 4 - Fraction(1, 4)
...
def get_grace_note(note, beat):
    if note.effect.grace is None:
        ...
        if guitarpro.models.GraceEffectTransition.bend == note.effect.grace.transition:
            transition = "bend"
        if (
            guitarpro.models.GraceEffectTransition.hammer
        ...
        grace_note = datatypes.GraceNote(
            note.effect.grace.fret,
            grace_duration,
            get_note_dynamic(note.effect.grace, beat),
            note.effect.grace.isDead,
            note.effect.grace.isOnBeat,
            transition,
        )
...
def get_plucking_adornment(beat, note):
    ...
    return datatypes.PluckingAdornment(
        get_plucking_technique(beat, note), get_plucking_modification(note), accent
    )
...
def get_artificial_harmonic_data(note):
    if type(note.effect.harmonic) == guitarpro.models.ArtificialHarmonic:
        return datatypes.ArtificialHarmonic(
            note.effect.harmonic.octave, note.effect.harmonic.pitch
        )
    else:
        return 0
...
def get_fretting_adornment(beat, note, hammerpullinfo):
    ...
    return datatypes.FrettingAdornment(
        ...
        get_fretting_accent(note.effect),
        get_modulation(note),
    )
...
def get_trill(note):
    ...
    return datatypes.Trill(
        note.effect.trill.fret, get_notated_duration(note.effect.trill.duration)
    )
    #                       datatypes.Duration(
...
def get_measure_data(
    song, measure_to_get_data_for, hampullfrom=[], slide_from=[], previous_beat_notes=[]
):
                            ...
                            if beat.status != guitarpro.BeatStatus.empty:
                                ...
                                for note in beat.notes:
                                    ...
                                    if note.effect.hammer == True:
                                        ...
                                        hampullfrom.append([note.string, note.value])
    ...
    return (
        datatypes.Measure(measure_meta_data, measure_start_time, measure_notes),
        hampullfrom,
        slide_from,
        previous_beat_notes,
    )
...
def get_measure_meta_data(song, measure):
    return datatypes.MeasureMetaData(
        get_song_title(song),
        get_measure_number(measure),
        get_key_signature(measure),
        get_timesignature(measure),
        # get_tempo(measure), get_triplet_feel(measure), None, None, None)
        song.tempo,
        get_triplet_feel(measure),
        None,
        None,
        None,
    )
...
def get_timesignature(measure):
    return (
        str(measure.header.timeSignature.numerator)
        + "/"
        + str(measure.header.timeSignature.denominator.value)
    )
...
def get_song_data(song):
    ...
    if len(get_bass_track_numbers(song)) == 1:
        song_data = datatypes.Song(get_song_meta_data(song), get_song_measures(song))
...
def get_song_meta_data(song, multi_track=False):
    ...
    if multi_track is False:
        return datatypes.SongMetaData(
            ...
            get_song_title(song),
            song.tempo,
            str(song.key).split(".")[1],
            len(song.tracks[0].measures),
        )
    else:
        return datatypes.SongMetaData(
            get_song_artist(song),
            get_song_title(song) + "_track_" + str(multi_track),
            song.tempo,
            str(song.key).split(".")[1],
            len(song.tracks[0].measures),
        )
            ...
            if utilities.midi2BassType.get(track.channel.instrument):
                ...
                for measure in track.measures:
                    ...
                    measure_data = get_measure_data(
                        song, measure, hampullfrom, slide_from, previous_beat_notes
                    )
