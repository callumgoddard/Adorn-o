...
def merge_synpy_and_fantastic_features(
    ...
    for mcsv_file in mcsv_file_list:
        try:
            f = fantastic_interface.compute_features([mcsv_file], write_out=False)
    ...
    try:
        all_features = robjects.r["merge"](
            fantastic_features, get_synpy_data(rhy_table_output)
        )
...
def combine_feature_tables(feature_table1, feature_table2):
    ...
    updated_feature_dataframe = robjects.r["rbind"](feature_table1, feature_table2)
...
def combine_synpy_and_fantastic_features_for_two_notes(
    measure, two_notes, rhy_file, r_data_frame=False, save_table=False
):
    ...
    combined_features = [
        SynPy_features[0],
        FANTASTIC_features["p.range"],
        FANTASTIC_features["p.entropy"],
        FANTASTIC_features["p.std"],
        FANTASTIC_features["i.abs.mean"],
        FANTASTIC_features["i.abs.std"],
        FANTASTIC_features["i.mode"],
        FANTASTIC_features["i.entropy"],
        FANTASTIC_features["d.range"],
        FANTASTIC_features["d.median"],
        FANTASTIC_features["d.mode"],
        FANTASTIC_features["d.entropy"],
        FANTASTIC_features["len"],
        FANTASTIC_features["glob.duration"],
        FANTASTIC_features["note.dens"],
    ] + SynPy_features[1:]
