...
def subdivide(seq, divisor):
    ...
    if len(seq) % divisor != 0:
        ...
        for i in range(divisor):
            subSeq.append(seq[int(start) : int(end)])
            start = end
            end = end + n
    return subSeq
...
def upsample_velocity_sequence(velocitySequence, length):
    ...
    if length < len(velocitySequence):
        print(
            "Error: the requested upsampling length needs to be longer than velocity sequence."
        )
    elif length % len(velocitySequence) != 0:
        print(
            "Error: velocity sequence can only be upsampled to a interger times of its own length."
        )
    else:
        ...
        if level == 0:
            ...
            for h in H_pre:
                H = concatenate(
                    ...
                    concatenate(
                        [h],
                        repeat([weightSequence[level]], subdivisionSequence[level] - 1),
                    ),
                )
    else:
        print(
            "Error: a subdivision factor or metrical weight is not defined for the request metrical level."
        )
    return H
...
def get_rhythm_category(velocitySequence, subdivisionSequence):
    ...
    """
    rhythmCategory = "mono"
    for f in find_prime_factors(
        len(velocity_sequence_to_min_timespan(velocitySequence))
    ):
...
def find_rhythm_Lmax(rhythmSequence, Lmax, weightSequence, subdivisionSequence):
    ...
    if needBiggerLmax:
        print(
            "Error: needs a bigger L_max (i.e. the lowest metrical level) to match the given rhythm sequence."
        )
        L = None
