...
def get_bars_from_midi(midiFile):
    ...
    def get_time_signature(
        timeList, barStartTime, barLength, ticksPerQuarter, currentTimeSignature=None
    ):
        ...
        while i < len(timeList):
            ...
            if event.type == "TIME_SIGNATURE" and event.time >= barStartTime:
                ...
                barLength = calculate_bar_ticks(
                    timesig.get_numerator(), timesig.get_denominator(), ticksPerQuarter
                )
    ...
    while noteIndex < len(notesList):
        ...
        [timesig, barlength] = get_time_signature(
            timeList, barStartTime, barlength, ticksPerQuarter, timesig
        )
    ...
    while index < len(noteEventList):
        ...
        if event.type == "NOTE_ON" and event.velocity != 0:
            ...
            while localindex < len(noteEventList):
                ...
                if endEvent.pitch == event.pitch and (
                    endEvent.type == "NOTE_OFF"
                    or (endEvent.type == "NOTE_ON" and endEvent.velocity == 0)
                ):
                    # make a note
...
def get_note_events(midiFile, trackNumber=None):
    ...
    return get_events_of_type(
        midiFile, trackNumber, lambda e: (e.type == "NOTE_ON") | (e.type == "NOTE_OFF")
    )
...
def get_time_events(midiFile):
    ...
    """
    return get_events_of_type(
        midiFile, 0, lambda e: (e.type == "SET_TEMPO") | (e.type == "TIME_SIGNATURE")
    )
...
def get_events_of_type(midiFile, trackIndex, lambdaEventType):
    ...
    while localEventFound == True:
        ...
        (localEventIndex, localEventDelta, localEventFound) = find_event(
            track, eventIndex, lambdaEventType
        )
...
def midi_event_to_time_signature(midiTimeSignatureEvent):
    ...
    if midiTimeSignatureEvent.type != "TIME_SIGNATURE":
        print(
            "Error in midi_event_to_time_signature(),  event must be a midi time signature type"
        )
        return None
    else:
