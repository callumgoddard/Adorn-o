        ...
        if timeSignature == None:
            ...
            if field != None:
                ...
                if fieldname.lower() == "t":
                    ...
                    print(
                        "Error, first field in the file should set the time signature."
                    )

        # parse the line
        (newbarlist, tempo, timeSignature, ticksPerQuarter) = parse_line(
            line, timeSignature, tempo, ticksPerQuarter
        )
    ...
    while len(line) > 0:
        ...
        if field != None:
            ...
            if fieldname.lower() == "v":
                # velocity sequence
                bar = Bar(
                    VelocitySequence(value), timeSignature, ticksPerQuarter, tempo
                )
                bars.append(bar)
